...
describe("run commands", () => {
  ...
  it("allows an eighth deployed hero but rejects a ninth", async () => {
    ...
    await repository.save({
      ...
      level: 8, experience: 0, commandResponses: {}, bench: [eighthHero, ninthHero], board: [...deployedHeroes, ...Array(5).fill(null)],
    });

    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-eight-hero-cap", commandId: "cmd-deploy-eighth", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: eighthHero.instanceId, destination: 19 }, repository))
      .resolves.toEqual({ runRevision: 1, status: "APPLIED" });
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-eight-hero-cap", commandId: "cmd-deploy-ninth", expectedRevision: 1, type: "MOVE_HERO", heroInstanceId: ninthHero.instanceId, destination: 20 }, repository))
      .rejects.toThrow("GAME_RULE_VIOLATION");
  });
  ...
  it("attaches the active Cow low-health trait trigger to counted board holders", async () => {
    ...
    const result = runHeadlessCombat({
      ...
      units: snapshot.units.map((unit, index) => {
        if (unit.side === "player") return { ...unit, position: 12 + index, attackSpeed: 0 };
        const currentEnemyIndex = enemyIndex++;
        return {
          ...unit,
          position: 8 + currentEnemyIndex,
  ...
  it("executes the active Cat first-critical trait buff from the locked breakpoint", async () => {
    ...
    const result = runHeadlessCombat({
      ...
      units: snapshot.units.map((unit, index) => {
        ...
        const currentEnemyIndex = enemyIndex++;
        return { ...unit, position: 8 + currentEnemyIndex, attackSpeed: 0 };
      }),
    });
  ...
  it("rejects destination 32 and non-integer destinations without mutation", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-boundary", commandId: "cmd-destination-32", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 24 }, repository)).rejects.toThrow("GAME_RULE_VIOLATION");
  ...
  it("rejects a board hero sent to its current cell without mutation", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-current-cell", commandId: "cmd-current-cell", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 24 }, repository)).rejects.toThrow("GAME_RULE_VIOLATION");
  ...
  it("rejects invalid destinations and heroes not owned by the run without mutation", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-invalid-move", commandId: "cmd-not-owned", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: "hero-not-owned", destination: 24 }, repository)).rejects.toThrow("GAME_RULE_VIOLATION");
