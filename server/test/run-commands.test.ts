...
describe("run commands", () => {
  ...
  it("enforces the level-derived board cap for placement and round start", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-level-board-cap", commandId: "cmd-over-cap-move", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: heroes[4]!.instanceId, destination: 20 }, repository))
      .rejects.toThrow("GAME_RULE_VIOLATION");
  ...
  it("allows an eighth deployed hero but rejects a ninth", async () => {
    const module = await import(modulePath) as typeof import("../src/application/run-commands.js");
    const repository = module.createInMemoryRunRepository();
    const deployedHeroes = Array.from({ length: 7 }, (_, index) => ({ instanceId: `hero-deployed-${index + 1}`, heroId: `H0${index + 1}`, cost: 1 }));
    const eighthHero = { instanceId: "hero-eighth", heroId: "H08", cost: 1 };
    const ninthHero = { instanceId: "hero-ninth", heroId: "H09", cost: 1 };
    await repository.save({
      id: "run-eight-hero-cap", tenantId: "tenant-a", contentVersion: "alpha-0.3.0", state: "PREPARE", round: 8, revision: 0, gold: 8,
      level: 8, experience: 0, commandResponses: {}, bench: [eighthHero, ninthHero], board: [...deployedHeroes, ...Array(9).fill(null)],
    });

    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-eight-hero-cap", commandId: "cmd-deploy-eighth", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: eighthHero.instanceId, destination: 23 }, repository))
      .resolves.toEqual({ runRevision: 1, status: "APPLIED" });
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-eight-hero-cap", commandId: "cmd-deploy-ninth", expectedRevision: 1, type: "MOVE_HERO", heroInstanceId: ninthHero.instanceId, destination: 24 }, repository))
      .rejects.toThrow("GAME_RULE_VIOLATION");
  });
  ...
  it("attaches the active Cow low-health trait trigger to counted board holders", async () => {
    ...
    const result = runHeadlessCombat({
      ...
      units: snapshot.units.map((unit, index) => {
        if (unit.side === "player") return { ...unit, position: 16 + index, attackSpeed: 0 };
  ...
  it("executes the active Cat first-critical trait buff from the locked breakpoint", async () => {
    ...
    const result = runHeadlessCombat({
      ...
      units: snapshot.units.map((unit, index) => {
        ...
        const currentEnemyIndex = enemyIndex++;
        return { ...unit, position: 12 + currentEnemyIndex, attackSpeed: 0 };
      }),
    });
    ...
    const snapshot = adapter.buildCombatSnapshot({
      ...
      lockedSnapshot: {
        ...
        board: [
          { instanceId: "dog-a", heroId: "H06", cost: 1 }, { instanceId: "dog-b", heroId: "H07", cost: 2 },
          { instanceId: "dog-c", heroId: "H08", cost: 2 }, { instanceId: "dog-d", heroId: "H09", cost: 3 },
          { instanceId: "dog-e", heroId: "H10", cost: 2 }, ...Array(11).fill(null),
        ],
      },
    ...
    const snapshot = adapter.buildCombatSnapshot({
      ...
      lockedSnapshot: {
        ...
        board: [
          ...
          { instanceId: "cat-e", heroId: "H05", cost: 2 }, ...Array(11).fill(null),
        ],
      },
  ...
  it("moves a bench hero into an empty global board destination", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-move", commandId: "cmd-move", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 16 }, repository))
      .resolves.toEqual({ runRevision: 1, status: "APPLIED" });
  ...
  it("swaps heroes between occupied board cells", async () => {
    ...
    await module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-board-swap", commandId: "cmd-board-swap", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: first.instanceId, destination: 17 }, repository);
  ...
  it("returns the displaced board hero to the bench when swapping a bench hero", async () => {
    ...
    await module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-bench-swap", commandId: "cmd-bench-swap", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: benchHero.instanceId, destination: 16 }, repository);
  ...
  it("rejects a bench move into an empty board cell at the round-one capacity without mutation", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-capacity", commandId: "cmd-capacity", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: benchHero.instanceId, destination: 19 }, repository))
      .rejects.toThrow("GAME_RULE_VIOLATION");
  ...
  it("allows a fourth hero from the bench onto an empty board cell in round two", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-round-two-capacity", commandId: "cmd-round-two-capacity", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: benchHero.instanceId, destination: 19 }, repository))
      .resolves.toEqual({ runRevision: 1, status: "APPLIED" });
  ...
  it("rejects destination 32 and non-integer destinations without mutation", async () => {
    const module = await import(modulePath) as typeof import("../src/application/run-commands.js");
    const repository = module.createInMemoryRunRepository();
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-boundary", commandId: "cmd-destination-32", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 32 }, repository)).rejects.toThrow("GAME_RULE_VIOLATION");
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-boundary", commandId: "cmd-destination-fraction", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 16.5 }, repository)).rejects.toThrow("GAME_RULE_VIOLATION");
  ...
  it("rejects a board hero sent to its current cell without mutation", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-current-cell", commandId: "cmd-current-cell", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 16 }, repository)).rejects.toThrow("GAME_RULE_VIOLATION");
  ...
  it("rejects invalid destinations and heroes not owned by the run without mutation", async () => {
    ...
    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-invalid-move", commandId: "cmd-not-owned", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: "hero-not-owned", destination: 16 }, repository)).rejects.toThrow("GAME_RULE_VIOLATION");
  ...
  it("rejects reuse of a move command ID for a different destination", async () => {
    ...
    await module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-move-idempotent", commandId: "cmd-move-idempotent", expectedRevision: 0, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 16 }, repository);

    await expect(module.applyRunCommand({ actorId: "actor-a", tenantId: "tenant-a", runId: "run-move-idempotent", commandId: "cmd-move-idempotent", expectedRevision: 1, type: "MOVE_HERO", heroInstanceId: hero.instanceId, destination: 17 }, repository))
      .rejects.toThrow("IDEMPOTENCY_KEY_REUSED");
  });
