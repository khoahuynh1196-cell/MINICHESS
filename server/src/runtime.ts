...
export function loadCompiledRuleset(rulesPath: string): CompiledRuleset {
  return compileRuleset(JSON.parse(readFileSync(rulesPath, "utf8")));
}

export function createContentShopGenerator(content: CompiledContentBundle, ruleset: CompiledRuleset): ShopGenerator {
  return Object.freeze({
    createPool: (input: Pick<CreateRunInput, "id" | "contentVersion" | "rulesetVersion"> & { readonly runSeed: string }) => {
      if (input.rulesetVersion !== ruleset.version) throw new Error("RULESET_VERSION_MISMATCH");
      return createShopPool(content, input.runSeed, ruleset.shop);
    },
    rollShop: (pool: ShopPool, input: { readonly round: number; readonly refreshNumber: number; readonly level: number }) =>
      rollShop(pool, input.level, `shop:${input.round}:${input.refreshNumber}`, ruleset.shop),
  });
}
...
export async function createRuntimeApp(options: RuntimeOptions) {
  ...
  const ruleset = loadCompiledRuleset(options.rulesPath);
  validateContentAgainstRuleset(content, ruleset);
  ...
  return createHttpApp(
    ...
    createContentShopGenerator(content, ruleset),
    contentRepository,
    ruleset,
  );
}
