...
func _init() -> void:
	...
	if manifest.has_method("hero_profile"):
		...
		_expect(unit.position.is_equal_approx(ArenaProjectionScript.cell_center(12)), "player slot 12 must begin the lower half of the portrait 4x6 arena")
		unit.move_to(11)
		_expect(unit.position.is_equal_approx(ArenaProjectionScript.cell_center(11)), "enemy slot 11 must remain in the upper half of the portrait 4x6 arena")
		unit.move_to(12)
	...
	for biome_id in ["meadow", "ruins", "frost_keep", "ember_citadel"]:
		...
		if layer != null and layer.texture != null:
			...
			_expect(layer.position.is_equal_approx(ArenaProjectionScript.texture_position()), "%s biome art must be centered in the 4x6 board" % biome_id)
			_expect(layer.scale.is_equal_approx(ArenaProjectionScript.texture_scale(layer.texture.get_size())), "%s biome art must be scaled to cover the 4x6 board" % biome_id)
	...
	if controller.has_method("combat_layout"):
		...
		_expect(controls_rect.position.y <= 1100.0, "the replay control rail must begin directly after the 4x6 board while preserving the portrait footer")
