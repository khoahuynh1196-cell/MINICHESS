...
func _init() -> void:
	...
	if manifest.has_method("hero_profile"):
		...
		unit.call("configure", "player", 16, 100000, h20_profile)
		_expect(unit.hero_id == "H20", "UnitView must consume the resolved profile rather than a color-only hero ID")
		_expect(unit.position.is_equal_approx(Vector2(165.0, 730.0)), "player slot 16 must begin the lower half of the portrait 4x8 arena")
		unit.move_to(15)
		_expect(unit.position.is_equal_approx(Vector2(915.0, 610.0)), "enemy slot 15 must remain in the upper half of the portrait 4x8 arena")
		unit.move_to(16)
	...
	for biome_id in ["meadow", "ruins", "frost_keep", "ember_citadel"]:
		...
		if layer != null and layer.texture != null:
			...
			_expect(layer.position.is_equal_approx(Vector2(540.0, 670.0)), "%s biome art must be centered in the 4x8 board" % biome_id)
			_expect(layer.scale.is_equal_approx(Vector2(1000.0 / layer.texture.get_size().x, 960.0 / layer.texture.get_size().y)), "%s biome art must be scaled to cover the 4x8 board" % biome_id)
	...
	if controller.has_method("combat_layout"):
		...
		_expect(controls_rect.position.y <= 1184.0, "the replay control rail must begin directly after the 4x8 board")
	...
	status_unit.configure("player", 16, 100000, manifest.call("hero_profile", "H01"))
