...
func _init() -> void:
	...
	_expect(controller.request_move("hero-a", 12, "PREPARE"), "the first global player board target must be emitted in prepare")
	_expect(controller.request_move("hero-a", 23, "PREPARE"), "the final global player board target must be emitted in prepare")
	...
	_expect(controller.request_drag_drop("hero-c", 22, "PREPARE"), "drag drop must emit the same move intent")
	_expect(not controller.request_move("hero-a", 11, "PREPARE"), "the last enemy target must be rejected")
	_expect(not controller.request_move("hero-a", 24, "PREPARE"), "the target after the player half must be rejected")
	...
	_expect(requests == [{ "hero": "hero-a", "destination": 12 }, { "hero": "hero-a", "destination": 23 }, { "hero": "hero-a", "destination": 0 }, { "hero": "hero-b", "destination": 13 }, { "hero": "hero-c", "destination": 22 }], "only legal formation requests must be emitted")
	if _failed:
		quit(1)
		return
