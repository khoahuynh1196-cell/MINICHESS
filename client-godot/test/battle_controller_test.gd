...
func _init() -> void:
	...
	_expect(controller.unit_views["player:H01:1"].get("animation_state") == "hit", "lower-priority shield VFX must not overwrite an active hit reaction")
	controller.apply_event(_event("CAST_STARTED", "player:H01:1", "enemy:E01:1", {}))
	_expect(controller.unit_views["player:H01:1"].get("animation_state") == "hit", "lower-priority cast VFX must not overwrite an active hit reaction")
	controller.apply_event(_event("STUN_APPLIED", "enemy:E01:1", "player:H01:1", {}))
	_expect(controller.unit_views["player:H01:1"].get("animation_state") == "control", "stun effects must present a target hard-control reaction")
