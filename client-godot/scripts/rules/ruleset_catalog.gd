...
static func _is_valid_ruleset(rules: Dictionary) -> bool:
	if String(rules.get("version", "")).is_empty():
		...
		return false
	var combat = rules.get("combat", null)
	var board = rules.get("board", null)
	var roster = rules.get("roster", null)
	var shop = rules.get("shop", null)
	var progression = rules.get("progression", null)
	var adventure = rules.get("adventure", null)
	if not combat is Dictionary or not board is Dictionary or not roster is Dictionary \
		or not shop is Dictionary or not progression is Dictionary or not adventure is Dictionary:
	...
	if int(board.get("columns", 0)) <= 0 or int(board.get("rows", 0)) <= 0 or String(board.get("movement", "")) != "orthogonal":
		...
		return false
	if not board.get("enemy_rows", null) is Dictionary or not board.get("player_rows", null) is Dictionary:
	...
	if int(roster.get("bench_slots", 0)) <= 0 or int(roster.get("max_items_per_hero", 0)) <= 0 or int(roster.get("max_unique_per_team", 0)) <= 0:
		...
		return false
	if int(shop.get("slot_count", 0)) <= 0 or not shop.get("odds_by_level", null) is Dictionary:
		push_error("Ruleset shop values are invalid")
		return false
	if int(progression.get("initial_level", 0)) <= 0 or int(progression.get("max_level", 0)) <= 0 or not progression.get("levels", null) is Array:
